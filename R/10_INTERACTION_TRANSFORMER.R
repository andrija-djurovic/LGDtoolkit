...
#'@param target Name of target variable within db argument.
#'@param min.pct.obs Minimum percentage of observation in each leaf.
#'@param min.avg.rate Minimum average target rate in each leaf..
...
#'library(LGDtoolkit)
#'data(lgd.ds.c)
#'it <- interaction.transformer(db = lgd.ds.c,
#'			              rf = c("rf_01", "rf_03"), 
#'                              target = "lgd",
#'                              min.pct.obs = 0.05,
#'                              min.avg.rate = 0.01,
...
best.split.num <- function(tbl, rf.l, target, min.obs, min.rate, monodir) {
	...
	tbl.s <- tbl %>%
		   ...
		   mutate(n.cs = cumsum(n),
			    ...
			    loss.c = y.cs / n.cs,
			    loss.r = (sum(y.sum) - y.cs) / (sum(n) - n.cs))
	...
	if	(length(monodir) > 0) {
		if	(monodir == 1) {
			ms <- which(tbl.s$loss.c < tbl.s$loss.r)
			} else {
			ms <- which(tbl.s$loss.c > tbl.s$loss.r)
			}
		} else {
	...
	for	(i in 1:sl) {
		split.l <- splits[i]
		ssv[i] <- sse.num(y = tbl[, target], x = tbl[, rf.l], sp = split.l)
		}
	split.at <- splits[which.min(ssv)]
...
best.split.cat <- function(tbl, rf.l, target, min.obs, min.rate) {
	...
	tbl.s <- tbl %>%
		   ...
		   mutate(n.cs = cumsum(n),
			    ...
			    loss.c = y.cs / n.cs,
			    loss.r = (sum(y.sum) - y.cs) / (sum(n) - n.cs))
	lt <- min(which(tbl.s$n.cs >= min.obs & tbl.s$y.cs.a >= min.rate)) 
	...
	for	(i in 1:sl) {
		...
		ssv[i] <- sse.cat(y = tbl[, target], x = tbl[, rf.l], sp = split.l)
		}
	split.at <- splits[which.min(ssv)]
...
#sse
sse.num <- function(y, x, sp) {
	res <- sum((y[x < sp] - mean(y[x < sp]))^2) +
		 sum((y[x >= sp] - mean(y[x >= sp]))^2) 
return(res)	
}
sse.cat <- function(y, x, sp) {
	res <- sum((y[x%in%sp] - mean(y[x%in%sp]))^2) +
		 sum((y[!x%in%sp] - mean(y[!x%in%sp]))^2) 
return(res)	
}
#interactions
